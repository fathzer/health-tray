...
public class StatePersistence {
	 ...
	 */
	public void save(List<CheckTask> tasks) throws IOException {
		Properties props = new Properties();
		props.setProperty("task.count", String.valueOf(tasks.size()));
		for (int i = 0; i < tasks.size(); i++) {
			CheckTask task = tasks.get(i);
			CheckTask.SavedState state = task.captureState();
	 ...
	 * @return a map of task name to {@link CheckTask.SavedState}, or an empty map if the file doesn't exist.
	 * @throws IOException if the file cannot be read.
	 */
	public Map<String, CheckTask.SavedState> load() throws IOException {
		...
		Map<String, CheckTask.SavedState> result = new HashMap<>();
		for (int i = 0; i < count; i++) {
			...
			CheckTask.Status status = statusStr.isEmpty() ? null : CheckTask.Status.valueOf(statusStr);
			...
			result.put(name, new CheckTask.SavedState(name, status, message, lastCheck, lastChange));
		}
		return result;
	}
}
